import type { Category, Transaction } from "@coco/shared";
...
export default function CategoryDetailScreen() {
  ...
  const { data: allTransactions = [], isLoading: txLoading } =
    useMonthlyTransactions(displayYear, displayMonth);
  ...
  const sortedCategories = useMemo(() => {
    ...
    const expenses = mapped
      ...
      .sort(
        (a, b) => (EXPENSE_ORDER[a.name] ?? 99) - (EXPENSE_ORDER[b.name] ?? 99),
      );
    const incomes = mapped
      .filter((c) => c.type === "income")
      .sort(
        (a, b) => (INCOME_ORDER[a.name] ?? 99) - (INCOME_ORDER[b.name] ?? 99),
      );
    return [...expenses, ...incomes];
  }, [categories]);

  const filteredTransactions =
    activeFilter === ALL_EXPENSE
