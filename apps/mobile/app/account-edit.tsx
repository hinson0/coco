...
export default function AccountEditScreen() {
  ...
  const initialTypeConfig =
    (isEdit && params.type === "e_wallet"
      ? TYPE_OPTIONS.find((t) => t.brandKey === params.icon)
      : TYPE_OPTIONS.find((t) => t.dbType === params.type)) ?? TYPE_OPTIONS[0];

  const [selectedType, setSelectedType] =
    useState<TypeConfig>(initialTypeConfig);
  const [name, setName] = useState(params.name ?? "");
  const [icon, setIcon] = useState(
    params.icon ??
      initialTypeConfig.emoji ??
      initialTypeConfig.brandKey ??
      "📦",
  );
      ...
      <View style={styles.body}>
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.formRow}>
            <TouchableOpacity
              onPress={() =>
                selectedType.dbType === "custom" && setShowEmojiPicker(true)
              }
            ...
            <View style={{ flex: 1 }}>
              ...
              <TextInput
                ...
                maxLength={20}
                editable={
                  !selectedType.autoName || selectedType.dbType === "e_wallet"
                }
                multiline
                numberOfLines={1}
              />
