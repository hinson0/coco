...
function Cell({ label, value, valueColor, sub, onPress }: { label: string; value: string; valueColor: string; sub?: string; onPress?: () => void }) {
  const content = (
...
export function OverviewCard({ expense, income, balance, balanceRaw, budget, remaining, dailyAvg, budgetPercent, daysLeft, hasBudget, onPressBudget, subs }: OverviewCardProps) {
  ...
  return (
    <Card radius="xl" shadow="lg" padding={18}>
      <View style={styles.grid}>
        ...
        <Cell label="结余" value={balance} valueColor={balanceColor} sub={subs?.balance} />
      </View>
      <View style={styles.divider} />
      <View style={styles.grid}>
        <Cell label="本月预算" value={budget} valueColor={budgetValueColor} sub={subs?.budget} onPress={onPressBudget} />
        <Cell label="月剩余" value={remaining} valueColor={hasBudget ? colors.sage : colors.textLighter} sub={subs?.remaining} />
        <Cell label="剩余日均" value={dailyAvg} valueColor={hasBudget ? colors.lavender : colors.textLighter} sub={`还剩${daysLeft}天`} />
      </View>
      {hasBudget && (
