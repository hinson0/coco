...
export function categoryChangeRule(ctx: InsightContext): InsightItem[] | null {
  ...
  for (const cat of current) {
    ...
    const changePercent =
      prevAmount === 0 ? 100 : ((cat.amount - prevAmount) / prevAmount) * 100;
    const absDiff = Math.abs(cat.amount - prevAmount);

    if (
      Math.abs(changePercent) >= MIN_CHANGE_PERCENT &&
      absDiff >= MIN_CHANGE_AMOUNT
    ) {
  ...
  if (up) {
    ...
    results.push({
      ...
      navigation: {
        ...
        params: { categoryId: up.cat.categoryId },
      },
      meta: {
        categoryId: up.cat.categoryId,
        currentAmount: up.cat.amount,
        previousAmount: up.prevAmount,
        changePercent: pct,
      },
    });
  }
  ...
  if (down) {
    ...
    results.push({
      ...
      navigation: {
        route: "/category-detail",
        params: { categoryId: down.cat.categoryId },
      },
      meta: {
        categoryId: down.cat.categoryId,
        currentAmount: down.cat.amount,
        previousAmount: down.prevAmount,
        changePercent: -pct,
      },
    });
  }
